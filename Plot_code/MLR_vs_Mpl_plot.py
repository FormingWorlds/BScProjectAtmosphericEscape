...
df = pd.read_csv("Outputs/Mass_loss_rate_vs_M_pl.csv")
...
# Left side: 1 M_earth
for element in ["H2", "H2O", "N2", "CO2"]:
    for flux in ["1_F", "1000_F"]:
        subset = df[
            (df["Element"] == element) &
            (df["Planet mass"] == "1_M") &
            (df["Earth Flux"] == flux)
        ...
        ax1.plot(
            subset["Total impactor mass [kg/s]"],
...
fig.suptitle(r"Mass loss rate vs total impactor mass over a unit time for $q=3$")
plt.tight_layout()
plt.savefig('MLR_vs_Mpl.png', dpi=450, bbox_inches="tight")
plt.show()
